/**
 * Versioned configuration object for Codee.
 */
...
export interface AdvancedSettings {
  ...
  /** Max entries to keep in the embedding LRU cache. */
  embeddingCacheMaxEntries: number;
  /** Idle timeout before shutting down language servers. */
  languageServerIdleMs: number;
  /** Emit and store performance metrics. */
  performanceMetricsEnabled: boolean;
  /** Use a deterministic mock LLM for testing. */
  mockLlmEnabled: boolean;
  /** Enable developer tooling and debug panels. */
  developerMode: boolean;
  /** Maximum files to index before warning. */
  maxIndexFiles: number;
}

/** Settings merge sources. */
export type SettingsSource = "default" | "user" | "project" | "runtime";
