...
function validateSyntax(language, code) {
    if (language === "typescript" || language === "javascript") {
        const tempDir = fs_1.default.mkdtempSync(path_1.default.join(os_1.default.tmpdir(), "codee-polyglot-"));
        const tempFile = path_1.default.join(tempDir, language === "javascript" ? "check.js" : "check.ts");
        fs_1.default.writeFileSync(tempFile, code, "utf-8");
        const tscResult = (0, child_process_1.spawnSync)("tsc", ["--noEmit", "--pretty", "false", tempFile], { encoding: "utf-8" });
        if (tscResult.error) {
            ...
            try {
                ...
                const details = diagnostics.map((diag) => String(diag.messageText));
    ...
    if (language === "python") {
        const tempDir = fs_1.default.mkdtempSync(path_1.default.join(os_1.default.tmpdir(), "codee-polyglot-"));
        const tempFile = path_1.default.join(tempDir, "check.py");
        fs_1.default.writeFileSync(tempFile, code, "utf-8");
        const result = (0, child_process_1.spawnSync)("python", ["-m", "py_compile", tempFile], { encoding: "utf-8" });
        fs_1.default.rmSync(tempDir, { recursive: true, force: true });
        if (result.error) {
    ...
    if (language === "rust") {
        const tempDir = fs_1.default.mkdtempSync(path_1.default.join(os_1.default.tmpdir(), "codee-polyglot-"));
        const tempFile = path_1.default.join(tempDir, "check.rs");
        fs_1.default.writeFileSync(tempFile, code, "utf-8");
        const result = (0, child_process_1.spawnSync)("rustc", ["--emit=metadata", tempFile], { encoding: "utf-8" });
        fs_1.default.rmSync(tempDir, { recursive: true, force: true });
        if (result.error) {
    ...
    if (language === "go") {
        const tempDir = fs_1.default.mkdtempSync(path_1.default.join(os_1.default.tmpdir(), "codee-polyglot-"));
        const tempFile = path_1.default.join(tempDir, "check.go");
        const tempMod = path_1.default.join(tempDir, "go.mod");
        const hasPackage = /^\s*package\s+\w+/m.test(code);
        const hasTopLevelDecl = /\b(func\s+\w+\s*\(|type\s+\w+\s+|var\s+\w+|const\s+\w+|import\s+\()/m.test(code);
        const wrappedSnippet = hasTopLevelDecl
            ? code
            : `func _() {\n${code.split("\n").map((line) => `  ${line}`).join("\n")}\n}`;
        const goSource = hasPackage ? code : `package main\n\n${wrappedSnippet}`;
        fs_1.default.writeFileSync(tempFile, goSource, "utf-8");
        fs_1.default.writeFileSync(tempMod, "module codee-temp\n\ngo 1.20\n", "utf-8");
        const formatResult = (0, child_process_1.spawnSync)("gofmt", ["-l", tempFile], { encoding: "utf-8" });
        ...
        const details = ["Go syntax check passed."];
        ...
        const vetResult = (0, child_process_1.spawnSync)("go", ["vet", tempDir], { encoding: "utf-8" });
        fs_1.default.rmSync(tempDir, { recursive: true, force: true });
        if (vetResult.error) {
    ...
    if (language === "java") {
        const tempDir = fs_1.default.mkdtempSync(path_1.default.join(os_1.default.tmpdir(), "codee-polyglot-"));
        const tempFile = path_1.default.join(tempDir, "Check.java");
        ...
        fs_1.default.writeFileSync(tempFile, javaSource, "utf-8");
        const result = (0, child_process_1.spawnSync)("javac", ["-d", tempDir, tempFile], { encoding: "utf-8" });
        fs_1.default.rmSync(tempDir, { recursive: true, force: true });
        if (result.error) {
    ...
    if (language === "cpp") {
        const tempDir = fs_1.default.mkdtempSync(path_1.default.join(os_1.default.tmpdir(), "codee-polyglot-"));
        const tempFile = path_1.default.join(tempDir, "check.cpp");
        ...
        const cppSource = hasMain
            ...
            : `int main() {\n${code
                ...
                .map((line) => `  ${line}`)
                .join("\n")}\n  return 0;\n}`;
        fs_1.default.writeFileSync(tempFile, cppSource, "utf-8");
        const clangResult = (0, child_process_1.spawnSync)("clang++", ["-fsyntax-only", "-std=c++20", tempFile], { encoding: "utf-8" });
        if (clangResult.error) {
            const gppResult = (0, child_process_1.spawnSync)("g++", ["-fsyntax-only", "-std=c++20", tempFile], { encoding: "utf-8" });
            fs_1.default.rmSync(tempDir, { recursive: true, force: true });
            if (gppResult.error) {
