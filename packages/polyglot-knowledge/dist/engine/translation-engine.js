...
exports.translateWithEngine = translateWithEngine;
...
function translateWithEngine(from, to, code, options = {}) {
    ...
    const patterns = (0, pattern_detector_1.detectPatterns)(code, from);
...
function selectBestPattern(patterns) {
...
function generateTranslation(match, from, to, code, decisions) {
...
function translateListComprehension(match, from, to, code) {
...
function buildGoListLoop(source, item, transform, filter) {
...
function buildJavaListPipeline(source, item, transform, filter) {
    const filterPart = filter ? `.filter(${item} -> ${filter})` : "";
    return [
...
function buildCppListLoop(source, item, transform, filter) {
    const lines = [
        "#include <vector>",
...
function translateDecorator(match, from, to, code) {
...
function translateAsyncAwait(match, from, to, code) {
...
function translateDataclass(match, from, to, code, decisions) {
    ...
    if (from === "typescript" && to === "rust") {
        const fields = parseTypeScriptFields(body);
        const lines = fields.map((field) => `  ${field.name}: ${mapTypeScriptTypeToRust(field.type, decisions.anyStrategy)},`);
        return ["#[derive(Debug, Clone)]", `struct ${name} {`, lines.join("\n"), "}"]
            .filter(Boolean)
            .join("\n");
    }
...
function translateErrorHandling(match, from, to, code) {
    if (from === "python" && to === "typescript") {
...
function translateDictLiteral(match, from, to, code, decisions) {
    if (to === "go") {
...
function translateNoneHandling(match, from, to, code) {
    if (from === "python" && to === "rust") {
...
function translateIterators(to) {
    if (to === "python") {
...
function translatePatternMatching(to) {
    if (to === "python") {
        return [
            "match value:",
            "    case 0:",
...
function translateConcurrencyPatterns(to) {
    if (to === "python") {
        return [
            "async def main():",
...
function resolveDecisions(decisions, options) {
    const resolved = {};
...
function extractConceptsFromPatterns(patterns) {
    const map = {
    ...
    return Array.from(new Set(patterns.map((pattern) => map[pattern.id]).filter((value) => Boolean(value))));
}
function parseArrowFunction(input) {
...
function parseJavaLambda(input) {
    const arrowIndex = input.indexOf("->");
    if (arrowIndex === -1) {
        return null;
    }
...
function parseRustClosure(input) {
...
function extractSignature(from, name, code) {
    if (from === "python") {
...
function extractAwaitExpression(from, code) {
    if (from === "python") {
        const match = /await\s+([^\n]+)/.exec(code);
...
function stripGoSignature(signature) {
    if (!signature.trim()) {
        return "";
    }
    return signature
...
function parsePythonFields(body) {
    return body
        .split("\n")
        .map((line) => line.trim())
...
function parseTypeScriptFields(body) {
    return body
        .split("\n")
        .map((line) => line.trim())
...
function formatGoFieldName(name) {
    if (!name) {
        return "Field";
    }
    return name.charAt(0).toUpperCase() + name.slice(1);
}
function formatGoMapKey(key) {
...
function mapPythonTypeToTypeScript(type) {
...
function mapPythonTypeToRust(type, defaultValue) {
    const normalized = type.replace(/\s+/g, "").toLowerCase();
...
function mapPythonTypeToGo(type) {
    const normalized = type.replace(/\s+/g, "").toLowerCase();
...
function mapPythonTypeToJava(type) {
    const normalized = type.replace(/\s+/g, "").toLowerCase();
    if (normalized.startsWith("list[")) {
        return "List<Object>";
    }
...
function mapPythonTypeToCpp(type) {
    const normalized = type.replace(/\s+/g, "").toLowerCase();
    if (normalized.startsWith("list[")) {
...
function mapTypeScriptTypeToPython(type) {
    const normalized = type.replace(/\s+/g, "").toLowerCase();
...
function mapTypeScriptTypeToRust(type, anyStrategy) {
    const normalized = type.replace(/\s+/g, "").toLowerCase();
    if (normalized.startsWith("array")) {
        return "Vec<_>";
    }
...
function mapTypeScriptTypeToGo(type) {
    const normalized = type.replace(/\s+/g, "").toLowerCase();
    if (normalized.startsWith("array")) {
        return "[]any";
    }
...
function mapTypeScriptTypeToJava(type) {
    const normalized = type.replace(/\s+/g, "").toLowerCase();
    if (normalized.startsWith("array")) {
        return "List<Object>";
    }
...
function mapTypeScriptTypeToCpp(type) {
    const normalized = type.replace(/\s+/g, "").toLowerCase();
    if (normalized.startsWith("array")) {
        return "std::vector<int>";
    }
...
function mapRustTypeToPython(type) {
    const normalized = type.replace(/\s+/g, "").toLowerCase();
...
function mapRustTypeToTypeScript(type) {
    const normalized = type.replace(/\s+/g, "").toLowerCase();
    if (normalized.startsWith("vec")) {
        return "Array<unknown>";
    }
...
function mapRustTypeToJava(type) {
    const normalized = type.replace(/\s+/g, "").toLowerCase();
    if (normalized.startsWith("vec")) {
        return "List<Object>";
    }
...
function mapRustTypeToCpp(type) {
    const normalized = type.replace(/\s+/g, "").toLowerCase();
    if (normalized.startsWith("vec")) {
        return "std::vector<int>";
    }
...
function mapGoTypeToPython(type) {
    const normalized = type.replace(/\s+/g, "").toLowerCase();
    if (normalized.startsWith("[]")) {
        return "list";
    }
...
function mapGoTypeToTypeScript(type) {
    const normalized = type.replace(/\s+/g, "").toLowerCase();
    if (normalized.startsWith("[]")) {
        return "Array<unknown>";
    }
...
function mapJavaTypeToPython(type) {
    const normalized = type.replace(/\s+/g, "").toLowerCase();
    if (normalized.startsWith("list<")) {
        return "list";
    }
...
function mapJavaTypeToTypeScript(type) {
    const normalized = type.replace(/\s+/g, "").toLowerCase();
    if (normalized.startsWith("list<")) {
        return "Array<unknown>";
    }
...
function mapJavaTypeToRust(type) {
    const normalized = type.replace(/\s+/g, "").toLowerCase();
    if (normalized.startsWith("list<")) {
        return "Vec<_>";
    }
...
function mapJavaTypeToCpp(type) {
    const normalized = type.replace(/\s+/g, "").toLowerCase();
    if (normalized.startsWith("list<")) {
        return "std::vector<int>";
    }
...
function mapCppTypeToPython(type) {
    const normalized = type.replace(/\s+/g, "").toLowerCase();
...
function mapCppTypeToTypeScript(type) {
    const normalized = type.replace(/\s+/g, "").toLowerCase();
    if (normalized.startsWith("std::vector")) {
        return "Array<unknown>";
    }
...
function mapCppTypeToRust(type) {
    const normalized = type.replace(/\s+/g, "").toLowerCase();
    if (normalized.startsWith("std::vector")) {
        return "Vec<_>";
    }
