...
function buildCacheKey(language, code) {
    return `${language}:${hashCode(code)}`;
}
function cachePatterns(key, patterns) {
    patternCache.set(key, patterns);
    enforceCacheLimit();
    return patterns;
}
function refreshCacheEntry(key, patterns) {
...
function detectPythonPatterns(code) {
    const matches = [];
    const listComp = matchOnce(/\[(?<transform>[^\]\n]+?)\s+for\s+(?<item>\w+)\s+in\s+(?<source>[^\]\n]+?)(?:\s+if\s+(?<filter>[^\]\n]+))?\]/, code, (match) => {
    ...
    const decorator = matchOnce(/^@(?<decorator>[\w.]+(?:\([^)]*\))?)\s*\n\s*(?<async>async\s+)?(?<target>def|class)\s+(?<name>\w+)/m, code, (match) => {
        const groups = match.groups ?? {};
        return {
            id: "decorator",
            language: "python",
    ...
    const dataclass = matchOnce(/@dataclass\s*\n\s*class\s+(?<name>\w+)\s*:\s*\n(?<body>(?:\s+.+\n?)*)/m, code, (match) => {
        const groups = match.groups ?? {};
        return {
            id: "dataclass",
            language: "python",
    ...
    const errorHandling = matchOnce(/try:\s*\n(?<tryBlock>[\s\S]+?)\n\s*except\s+(?<error>[^:\n]+)(?:\s+as\s+(?<var>\w+))?:/m, code, (match) => {
        const groups = match.groups ?? {};
        return {
            id: "error-handling",
            language: "python",
...
function detectTypeScriptPatterns(code, language) {
    const matches = [];
    ...
    const decorator = matchOnce(/^@(?<decorator>[\w.]+(?:\([^)]*\))?)\s*\n\s*(?<export>export\s+)?(?<target>class|function)\s+(?<name>\w+)/m, code, (match) => {
        const groups = match.groups ?? {};
        return {
            id: "decorator",
            language,
    ...
    const dataclass = matchOnce(/(interface|class)\s+(?<name>\w+)\s*\{(?<body>[\s\S]*?)\}/m, code, (match) => {
        const groups = match.groups ?? {};
        return {
            id: "dataclass",
            language,
...
function detectRustPatterns(code) {
    const matches = [];
...
function detectGoPatterns(code) {
    const matches = [];
    ...
    const listLoop = matchOnce(/for\s+_,\s*(?<item>\w+)\s*:=\s*range\s+(?<source>[\w.]+)\s*\{(?<body>[\s\S]*?)\}/m, code, (match) => {
        const groups = match.groups ?? {};
    ...
    const dataclass = matchOnce(/type\s+(?<name>\w+)\s+struct\s*\{(?<body>[\s\S]*?)\}/m, code, (match) => {
        const groups = match.groups ?? {};
        return {
            id: "dataclass",
            language: "go",
    ...
    const dictLiteral = matchOnce(/(?:(?<name>\w+)\s*:?=\s*)?map\[[^\]]+\][^{]*\{(?<body>[^}]+)\}/m, code, (match) => {
        const groups = match.groups ?? {};
        return {
            id: "dict-literal",
            language: "go",
...
function detectJavaPatterns(code) {
    const matches = [];
    const listComp = matchOnce(/(?<source>\w+)\.stream\(\)\.(?<first>filter|map)\((?<firstFn>[^)]+)\)\.(?<second>filter|map)\((?<secondFn>[^)]+)\)/, code, (match) => {
        const groups = match.groups ?? {};
...
function detectCppPatterns(code) {
    const matches = [];
...
function detectTypeScriptListChain(code, language) {
...
function detectRustIteratorChain(code) {
    const chain = /(?<source>\w+)\.(?<iter>iter|into_iter)\(\)\.(?<first>filter|map)\((?<firstFn>[^)]*)\)\.(?<second>filter|map)\((?<secondFn>[^)]*)\)\.collect/.exec(code);
    if (!chain || !chain.groups) {
        const single = /(?<source>\w+)\.(?<iter>iter|into_iter)\(\)\.map\((?<mapFn>[^)]*)\)\.collect/.exec(code);
        if (!single || !single.groups) {
            return null;
        }
        const singleSource = single.groups.source ?? "";
...
function matchOnce(regex, code, build) {
...
function scoreConfidence(base, ...signals) {
